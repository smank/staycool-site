...
test("sign → verify round-trip preserves all fields", async () => {
  const payload = buildPayloadV2({ product: "cartridge",
  ...
  assert.deepEqual(parsed.fields, {
    product: "cartridge", type: "full", name: "Jo Tester", email: "jo@example.com",
    order: "ls-1042", expiry: 0, machine: MACHINE_A,
  });
});

test("signing is deterministic", async () => {
  const payload = buildPayloadV2({ product: "cartridge", type: "beta", name: "A", email: "a@b.c", order: "beta-1", expiry: 20261101, machine: "*" });
...
test("buildPayloadV2 validation", () => {
  assert.throws(() => buildPayloadV2({ product: "cartridge", type: "site", name: "A", email: "", order: "o" }));
...
test("parseLicence rejects bad grammar", async () => {
  const good = await signLicence(TEST_PRIVATE_KEY, buildPayloadV2({ product: "cartridge", type: "full", name: "A", email: "", order: "o", machine: "*" }));
...
test("verifyLicence rejects tampered keys and wrong key", async () => {
  const key = await signLicence(TEST_PRIVATE_KEY, buildPayloadV2({ product: "cartridge", type: "full", name: "A", email: "", order: "o", machine: "*" }));
